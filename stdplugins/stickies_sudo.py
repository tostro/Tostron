...
@borg.on(admin_cmd(pattern="stick ?(.*)",allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    packname = f"Chat Sticks"
    packshortname = f"StickiesChat"  # format: Stickies_Chat
    ...
    if is_a_s:
        ...
        packname = f"Animated Sticks"
        packshortname = f"StickiesChat_as"  # format: Stickies_Chat
    ...
    meanii = await event.reply("Processing this sticker. Please Wait!")
    ...
    async with borg.conversation("@Stickers") as bot_conv:
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if "Yay!" not in response.text:
                await meanii.edit(f"**FAILED**! @Stickers replied: {response.text}")
                return
    ...
    await meanii.edit(f"sticker added! Get the pack [here](t.me/addstickers/{packshortname})")
