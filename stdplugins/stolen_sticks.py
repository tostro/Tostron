...
@borg.on(admin_cmd(pattern="steal ?(.*)",allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
        await event.edit("Reply to a photo to steal this sticker pack.")
        return
    ...
    packname = f"Stolen Stickers Inc."
    packshortname = f"StolenStickies"  # format: Stolen_Sticks

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@tostro.png"
    file = await borg.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await borg.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"Stolen Animated Sticks"
        packshortname = f"Stolen_as"  # format: Stolen_Sticks
    ...
    await event.edit("Stealing this sticker. Muhahaha!")
    ...
    await event.edit(f"sticker stolen hehehe! added to my collection [here](t.me/addstickers/{packshortname})")
